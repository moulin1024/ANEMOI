...
def anime(PATH, case_name):
    ...
    if config['ts_flag'] > 0:
        ...
        #     theta,r = np.meshgrid(azimuths,zeniths,indexing='ij')
            ...
            ax[0].set_xlabel('x')
            ax[0].set_ylabel('y')
            values = u[i,256,:,:]#np.random.random((azimuths.size, zeniths.size))
        ...
        anim = animation.FuncAnimation(fig, animate, frames=39)
        anim.save(out_path+'/animation.gif',writer='imagemagick', fps=20)
...
DEL_all = np.zeros([3,3])
# # end = 100000
# time_window = np.linspace(5000,100000,19)*0.02/60
# DEL_array = np.zeros(19)
# for k in range(19):
#     print(k)
#     for i in range(3):
...
for k in range(3):
    name = ["NREL-m-long","NREL-m-long-yaw-positive","NREL-m-long-yaw-negative"]
    f = h5py.File('./data/'+name[k]+'_force.h5','r')
    moment_flap = f['moment_flap'][()]

    for j in range(3):
        for i in range(3):
            m_f = moment_flap[:200000,0,i,j]/1e6
        ...
        print(DEL_avg)
        # plt.bar(S,N)
        # plt.bar(np.mean(S_all,axis=1),np.mean(N_all,axis=1),width=bin_width,alpha=0.5)
        DEL_all[j,k] = DEL_avg 

team = ['WT 1','WT 2','WT 3']
female = DEL_all[:,0]
male = DEL_all[:,1]
baseline = DEL_all[0,0]
x_axis = np.arange(len(team))

# Multi bar Chart

plt.bar(x_axis -0.2, DEL_all[:,0]/baseline, width=0.2, label = '$(0^\circ,0^\circ,0^\circ)$')
plt.bar(x_axis, DEL_all[:,1]/baseline, width=0.2, label = '$(-25^\circ,-15^\circ,0^\circ)$')
plt.bar(x_axis +0.2, DEL_all[:,2]/baseline, width=0.2, label = '$(25^\circ,15^\circ,0^\circ)$')
...
plt.savefig('plot/aligned.png')   



power = np.asarray([[4408787.,1960638.,2055314.],[3566541.,2544725.,2749065.],[3526306.,2552846.,2649717.]])    
baseline = power[0,0]
plt.figure(figsize=(10, 5), dpi=300)
plt.bar(x_axis -0.2, power[0,:]/baseline, width=0.2, label = '$(0^\circ,0^\circ,0^\circ)$')
plt.bar(x_axis, power[1,:]/baseline, width=0.2, label = '$(-25^\circ,-15^\circ,0^\circ)$')
plt.bar(x_axis +0.2, power[2,:]/baseline, width=0.2, label = '$(25^\circ,15^\circ,0^\circ)$')
...
# turbine = ['WT1','WT2','WT3']
# for k in range(2):
#     name = ["NREL-m-long-partial","NREL-m-long-partial-yaw"]
#     f = h5py.File('./data/'+name[k]+'_force.h5','r')
#     moment_flap = f['moment_flap'][()]
...
# plt.legend()
# plt.savefig('plot/aligned-yaw1.png')
...
# turbine = ['WT1','WT2','WT3']
# for k in range(2):
#     name = ["NREL-m-long-partial2","NREL-m-long-partial2-yaw"]
#     f = h5py.File('./data/'+name[k]+'_force.h5','r')
#     moment_flap = f['moment_flap'][()]
...
# plt.legend()
# plt.savefig('plot/aligned-yaw2.png')
